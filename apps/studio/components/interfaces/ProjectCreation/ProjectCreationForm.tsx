            ...
            !isVercelIntegrationFlow && (
              <div key="panel-title">
                ...
                {/* Note: in SuperBase² mode most fields on this form (region, instance size,
                    ...
                    used. Keeps upstream merge surface to a single block. */}
                <p className="text-sm text-foreground-lighter text-balance">
                  {SUPERBASE2_ENABLED ? (
                    <>
                      Creates a new database on your shared Postgres instance. Only{' '}
                      <strong>Project name</strong> is used — region, compute size, cloud provider,
                      high availability, Postgres version, and the database password field are all{' '}
                      <strong>ignored</strong> in self-hosted SuperBase² mode. The Postgres
                      superuser password is whatever <code>POSTGRES_PASSWORD</code> is set to in
                      your Coolify environment.
                    </>
                  ) : (
                    <>
